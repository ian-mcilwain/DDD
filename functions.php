...
add_action( 'wp_enqueue_scripts', 'ianM_scripts' );


/* Custom Title Tags */

function ianM_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
		$title = "$title $sep " . sprintf( __( 'Page %s', 'ianM' ), max( $paged, $page ) );
	}

	return $title;
}
add_filter( 'wp_title', 'ianM_wp_title', 10, 2 );

/*
  Get our wp_nav_menu() fallback, wp_page_menu(), to show a home link.
 */
function ianM_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'ianM_page_menu_args' );


/*
 * Sets the post excerpt length to 40 characters.
 */
function ianM_excerpt_length( $length ) {
	return 30;
}
add_filter( 'excerpt_length', 'ianM_excerpt_length' );

/*
 * Returns a "Continue Reading" link for excerpts
 */
function ianM_continue_reading_link() {
	return ' <a href="'. get_permalink() . '">Continue reading <span class="meta-nav">&rarr;</span></a>';
}

/**
 * Replaces "[...]" (appended to automatically generated excerpts) with an ellipsis and ianM_continue_reading_link().
 */
function ianM_auto_excerpt_more( $more ) {
	return ' &hellip;' . ianM_continue_reading_link();
}
add_filter( 'excerpt_more', 'ianM_auto_excerpt_more' );

/**
 * Adds a pretty "Continue Reading" link to custom post excerpts.
 */
function ianM_custom_excerpt_more( $output ) {
	if ( has_excerpt() && ! is_attachment() ) {
		$output .= ianM_continue_reading_link();
	}
	return $output;
}
add_filter( 'get_the_excerpt', 'ianM_custom_excerpt_more' );


/*
 * Register a single widget area.
 * You can register additional widget areas by using register_sidebar again
 * within ianM_widgets_init.
 * Display in your template with dynamic_sidebar()
 */
function ianM_widgets_init() {
...
add_action( 'widgets_init', 'ianM_widgets_init' );

/**
 * Removes the default styles that are packaged with the Recent Comments widget.
 */
function ianM_remove_recent_comments_style() {
	global $wp_widget_factory;
	remove_action( 'wp_head', array( $wp_widget_factory->widgets['WP_Widget_Recent_Comments'], 'recent_comments_style' ) );
}
add_action( 'widgets_init', 'ianM_remove_recent_comments_style' );


if ( ! function_exists( 'ianM_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post—date/time and author.
 */
function ianM_posted_on() {
...
if ( ! function_exists( 'ianM_posted_in' ) ) :
/**
 * Prints HTML with meta information for the current post (category, tags and permalink).
 */
function ianM_posted_in() {
